...
def get_video_rotation(file):
    ...
    stdout, stderr = p.communicate()
    video_lines = re.findall(
        "displaymatrix: rotation of -(.+) degrees",
        stderr.decode("utf-8", errors="ignore"),
    )
    if not video_lines:
        return False
        ...
        if metadata is not None:
            ...
            attrs.append(
                DocumentAttributeVideo(
                    ...
                    (0, video_meta.get("width" if not rotate else "height"))[
                        video_meta.has("width")
                    ],
                    (0, video_meta.get("height" if not rotate else "width"))[
                        video_meta.has("height")
                    ],
                    False,
                    True,
                )
